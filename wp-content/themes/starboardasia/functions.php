...
add_action( 'wp_ajax_nopriv_save_contact', 'prefix_save_contact_form' );
add_action( 'wp_ajax_save_contact', 'prefix_save_contact_form' );
function prefix_save_contact_form () {
    global $wpdb;
    ...
    update_post_meta($new_post_id, 'contact_phone', $_POST["phone"]);
    update_post_meta($new_post_id, 'contact_email', $_POST["email"]);
    update_post_meta($new_post_id, 'contact_address', $_POST["address"]);
...
add_action( 'wp_ajax_nopriv_save_recruit', 'prefix_save_recruit_form' );
add_action( 'wp_ajax_save_recruit', 'prefix_save_recruit_form' );
function prefix_save_recruit_form () {
    global $wpdb;
    ...
    update_post_meta($new_post_id, 'recruit_phone', $_POST["phone"]);
    update_post_meta($new_post_id, 'recruit_email', $_POST["email"]);
    update_post_meta($new_post_id, 'recruit_address', $_POST["address"]);
    update_post_meta($new_post_id, 'recruit_position', $_POST["position"]);
    update_post_meta($new_post_id, 'recruit_birth', $_POST["birth"]);

    if (!$_FILES["file_cv"]["error"])
	{
	    $file = $_FILES["file_cv"]["tmp_name"];	    
	    //perform the upload operation.
	    $name = strtotime(date('Y-m-d H:i:s')).'_'.basename($_FILES["file_cv"]["name"]);
	    move_uploaded_file( $file, "../uploads/". $name);
	    update_post_meta($new_post_id, 'recruit_file_csv', $name);
	}
...
// Recruitment post type function
function create_posttype_recruits() {
 
    register_post_type( 'recruit',
    // CPT Options
        array(
            'labels' => array(
                'name' => __( 'Recruitments' ),
...
// Hooking up our function to theme setup
add_action( 'init', 'create_posttype_recruits' );


add_filter( 'manage_recruit_posts_columns', 'smashing_recruit_columns' );
function smashing_recruit_columns( $columns ) {
 
    $columns = array(    
      'title' => "Name",
      'recruit_birth' => "Birthday",
      'recruit_phone' => "Phone",     
      'recruit_address' => "Address",
      'recruit_email' => "Email",
      'recruit_position' => "Position",
      'content' => "Note",
      'recruit_file_csv' => "File CV",
    );
...
add_action( 'manage_recruit_posts_custom_column', 'smashing_recruit_column', 10, 2);
function smashing_recruit_column( $column, $post_id ) {
	if ($column === 'content'){
		$post   = get_post( $post_id );
		$output =  apply_filters( 'the_content', $post->post_content );
		echo $output;
	}
	elseif ($column === 'recruit_file_csv'){
		echo '<a href="../uploads/'.get_post_meta( $post_id, $column, true ).'">'.get_post_meta( $post_id, $column, true ).'</a>';
